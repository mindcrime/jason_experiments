...
+!at(robot,P) : at(robot,P) <- true.   // at(X,Y) is a belief/percept from the Environment

+!at(robot,P) : not at(robot,P)        // at(X,Y) is a belief/percept from the Environment
  <- move_towards(P);               // move_towards is an action from the Environment
     !at(robot,P).                  // re-trigger this goal recursively until we arrive at the desired location
